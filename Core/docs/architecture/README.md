...
If you are working on self-build doctrine, Guardian-mediated build/delegation planning, coding-worker execution framing, Codex Runner alignment, or Pi-style execution positioning, start with [`Guardian Build Loop Doctrine`](./guardian-build-loop-doctrine.md) first. That document is the canonical umbrella doctrine for governance, delegation, execution, proof, and result return. For operational coding-worker details, continue into [`SOLO_OPERATOR_CODING_WORKER_RUNBOOK.md`](../Ops/SOLO_OPERATOR_CODING_WORKER_RUNBOOK.md).
...
- [Guardian Build Loop Doctrine](./guardian-build-loop-doctrine.md): canonical umbrella doctrine for self-build, delegation, coding-worker execution, Codex Runner alignment, Pi-style harness positioning, human review, proof posture, and duplication control. It consolidates existing bounded seams without widening release support or claiming new autonomous runtime behavior.
