...
def test_run_forever_retries_transient_dequeue_failures(monkeypatch):
    ...
    monkeypatch.setattr(
        chat_worker, "_publish_worker_heartbeat", lambda *_a, **_k: None
    )

    sleep_calls: list[float] = []
    monkeypatch.setattr(
        chat_worker.time, "sleep", lambda seconds: sleep_calls.append(seconds)
    )
    ...
    monkeypatch.setattr(chat_worker, "dequeue", fake_dequeue)
    ...
    assert not thread.is_alive(), "chat worker loop did not terminate"
...
def test_run_forever_surfaces_non_retryable_dequeue_errors(monkeypatch):
    ...
    monkeypatch.setattr(chat_worker.time, "sleep", lambda *_: None)
